...
Internal protocol dispatching and connection management. Forwards `TLSOptions` (`cert`, `key`, `pfx`, `passphrase`, `ca`) to the TLS engine.
...
Builds `multipart/form-data` request bodies per RFC 7578. Generates a cryptographically random boundary, serializes text fields and file attachments into a single `Buffer`. Content-Disposition values are escaped to prevent header injection.

Exported as `FormData`, `FormFile`, `FormValue` from the public API.
...
- `FormData` → `multipart/form-data` with boundary (body serialized via `FormData.encode()`).
- Plain object (`Record<string, unknown>`) → `application/json` (body serialized via `JSON.stringify`).
- String → `text/plain; charset=utf-8`.
- URLSearchParams → `application/x-www-form-urlencoded`.
- Buffer → no default Content-Type set.
- `ReadableStream` → must be pre-drained via `drainRequestBody()` before encoding; throws if passed directly.

The encoder also exports:

- `drainReadableStream(stream)` — reads all bytes from a `ReadableStream<Uint8Array>` into a `Buffer`.
- `drainRequestBody(request)` — if the request body is a `ReadableStream`, drains it; otherwise returns unchanged.

Header names and values are validated per RFC 7230 before encoding.
...
- Validates cookie names and values against control characters.
- Enforces Public Suffix List rules via `public-suffix.ts` (rejects Domain attributes that are public suffixes).
- Defaults `SameSite` to `lax` when the attribute is omitted.
- Validates `__Host-` prefix (requires `Secure`, no `Domain`, `Path=/`) and `__Secure-` prefix (requires `Secure`).

### `cookies/public-suffix.ts`

Public Suffix List implementation for cookie domain validation.

- Uses the complete Mozilla PSL (10,000+ rules) parsed into a trie at module load.
- Supports plain rules, wildcard rules (`*.uk`), and exception rules (`!www.ck`).
- Exported functions: `isPublicSuffix(domain)` and `getRegistrableDomain(hostname)`.
- Data file (`psl-data.ts`) generated by `npm run update-psl` and committed to the repo.
...
- `ProtocolError` with H2 RST_STREAM error codes 1 (PROTOCOL_ERROR), 2 (INTERNAL_ERROR), 7 (REFUSED_STREAM), 8 (CANCEL), 11 (ENHANCE_YOUR_CALM), 13 (HTTP_1_1_REQUIRED)

`NLcURLSession.request()` automatically invokes the retry helper when `retry.count > 0` is set in the session config. Exponential backoff factor is capped at 32× base delay.
...
Node TLS wrapper that tunes cipher/group/sigalgs/ALPN where supported. Forwards mTLS options (`cert`, `key`, `pfx`, `passphrase`, `ca`) from `TLSConnectOptions` to `tls.connect()`.
