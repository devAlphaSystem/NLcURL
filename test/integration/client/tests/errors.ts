...
export default async function () {
  ...
  await test("connection refused error", async () => {
    try {
      ...
      assert(err.message.includes("ECONNREFUSED") || err.message.includes("connect") || err.message.includes("timed out") || err.code === "ERR_CONNECTION", `Expected connection error, got: ${err.message}`);
    }
  });
  ...
  await test("unknown profile throws", async () => {
    try {
      ...
      assert(err.message.includes("Unknown browser profile") || err.code === "ERR_UNKNOWN_PROFILE", `Expected profile error, got: ${err.message}`);
    }
  });
