...
export default async function () {
  ...
  await test("request with timeout aborts slow response", async () => {
    try {
      ...
      assert(err.message.includes("timed out") || err.message.includes("timeout") || err.code === "ERR_TIMEOUT" || err.name === "TimeoutError", `Expected timeout error, got: ${err.message} (${err.code})`);
    }
  });
  ...
  await test("AbortController cancels request", async () => {
    ...
    try {
      ...
      assert(err.message.includes("abort") || err.name === "AbortError" || err.code === "ERR_ABORTED", `Expected abort error, got: ${err.message}`);
    }
  });
  ...
  await test("pre-aborted signal throws immediately", async () => {
    ...
    try {
      ...
      assert(err.message.includes("abort") || err.name === "AbortError", `Expected abort error, got: ${err.message}`);
    }
  });
}
