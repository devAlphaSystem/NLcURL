...
describe("instanceof chain", () => {
  it("all errors extend NLcURLError and Error", () => {
    const errors = [new TLSError("tls"), new HTTPError("http", 500), new TimeoutError("timeout", "total"), new ProxyError("proxy"), new AbortError(), new ConnectionError("conn"), new ProtocolError("proto")];
