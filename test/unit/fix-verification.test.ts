...
import { hkdfExtract, hkdfExpandLabel, deriveSecret, hashLength, zeroKey, deriveHandshakeKeys, deriveApplicationKeys, computeFinishedVerifyData, createHash as ksCreateHash } from "../../src/tls/stealth/key-schedule.js";
...
describe("Fix 1 – hkdfExpandLabel correctness (RFC 5869 / RFC 8446)", () => {
  it("produces known TLS 1.3 Early Secret → derived secret output", () => {
    ...
    let off = 0;
    hkdfLabel.writeUInt16BE(32, off);
    off += 2;
    hkdfLabel[off++] = fullLabel.length;
    fullLabel.copy(hkdfLabel, off);
...
describe("Fix 5 – JSON body defaults to application/json", () => {
  it("object body without explicit Content-Type uses application/json", () => {
    ...
    assert.ok(text.includes("content-type: application/json"), `Expected application/json but got:\n${text}`);
