...
function computeRedirect(
  ...
  body: string | null,
): { method: string; body: string | null; headers: Record<string, string> } {
...
describe('Fix 3 – redirect header stripping conditions', () => {
  ...
  it('307 preserves method, body, content-type, and content-length', () => {
    ...
    assert.equal(result.headers['content-type'], 'application/json',
      '307 must preserve content-type');
    assert.equal(result.headers['content-length'], '42',
      '307 must preserve content-length');
  });
  ...
  it('308 preserves method, body, content-type, and content-length', () => {
    ...
    assert.equal(result.headers['content-type'], 'application/json',
      '308 must preserve content-type');
    assert.equal(result.headers['content-length'], '42',
      '308 must preserve content-length');
  });
  ...
  it('302 + PUT keeps method and body (only POST→GET is converted)', () => {
    ...
    assert.equal(result.headers['content-type'], 'application/octet-stream',
      '302 + PUT should NOT strip content-type');
  });
});
