...
describe("HPACK encoder/decoder roundtrip", () => {
  ...
  it("encodes the same header consistently", () => {
    ...
    const headers1: Array<[string, string]> = [["x-request-id", "abc"]];

    const headers2: Array<[string, string]> = [["x-request-id", "abc"]];
  ...
  it("handles headers with empty values", () => {
    ...
    const headers: Array<[string, string]> = [[":authority", ""]];
...
describe("HPACK Huffman encoding", () => {
  ...
  it("Huffman-encoded output is smaller than plain text for typical headers", () => {
    ...
    const headers: Array<[string, string]> = [["x-long-header-name", "this-is-a-reasonably-long-header-value"]];
