...
describe("parseSetCookie", () => {
  ...
  it("parses cookie with attributes", () => {
    const cookie = parseSetCookie("id=42; Domain=example.com; Path=/; Secure; HttpOnly; SameSite=Strict", baseUrl);
    assert.ok(cookie);
  ...
  it("parses Expires", () => {
    const cookie = parseSetCookie("token=xyz; Expires=Thu, 01 Jan 2099 00:00:00 GMT", baseUrl);
    assert.ok(cookie);
...
describe("serializeCookies", () => {
  ...
  it("handles single cookie", () => {
    const result = serializeCookies([{ name: "x", value: "y", domain: "", path: "", secure: false, httpOnly: false, createdAt: 0 }]);
...
describe("CookieJar", () => {
  it("stores and retrieves cookies", () => {
    ...
    jar.setCookies({ "set-cookie": "session=abc; Path=/" }, new URL("https://example.com"));
  ...
  it("matches domain correctly", () => {
    const jar = new CookieJar();
    jar.setCookies({ "set-cookie": "id=1; Domain=example.com; Path=/" }, new URL("https://example.com"));
  ...
  it("matches path correctly", () => {
    const jar = new CookieJar();
    jar.setCookies({ "set-cookie": "x=1; Path=/api" }, new URL("https://example.com/api/test"));
  ...
  it("respects Secure flag", () => {
    const jar = new CookieJar();
    jar.setCookies({ "set-cookie": "secure=yes; Secure; Path=/" }, new URL("https://example.com"));
  ...
  it("clear() removes all cookies", () => {
    const jar = new CookieJar();
    jar.setCookies({ "set-cookie": "a=1; Path=/" }, new URL("https://example.com"));
  ...
  it("clearDomain() removes cookies for specific domain", () => {
    const jar = new CookieJar();
    jar.setCookies({ "set-cookie": "a=1; Path=/" }, new URL("https://example.com"));
    jar.setCookies({ "set-cookie": "b=2; Path=/" }, new URL("https://other.com"));
...
describe("CookieJar Netscape format", () => {
  it("toNetscapeString() produces valid format", () => {
    const jar = new CookieJar();
    jar.setCookies({ "set-cookie": "session=abc; Path=/; Domain=example.com" }, new URL("https://example.com"));
  ...
  it("loadNetscapeString() parses cookies", () => {
    const jar = new CookieJar();
    const content = ["# Netscape HTTP Cookie File", ".example.com\tTRUE\t/\tTRUE\t0\ttoken\txyz"].join("\n");
  ...
  it("roundtrips through Netscape format", () => {
    ...
    jar1.setCookies({ "set-cookie": "a=1; Path=/; Domain=example.com; Secure" }, new URL("https://example.com"));
    jar1.setCookies({ "set-cookie": "b=2; Path=/api; Domain=other.com" }, new URL("https://other.com/api/test"));
