...
describe('Fix 8 – rawHeaders preserve original header name casing', () => {
  it('parser rawHeaders retain original casing', () => {
    ...
    assert.ok(!result.rawHeaders.some(([k]) => k === 'content-type'),
      'rawHeaders should NOT have lowercased content-type');
    assert.ok(!result.rawHeaders.some(([k]) => k === 'x-request-id'),
      'rawHeaders should NOT have lowercased x-request-id');
  });
